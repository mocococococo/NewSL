...
def train_supervised(
    ...
    print(data_path)
    data_set = sorted(glob.glob(str(data_path)))
    if not data_set:
        raise FileNotFoundError(f"学習データが見つかりません: {data_path}")
    print("success to get data_set.")
    print(data_set)
    ...
    for epoch in range(epochs):
        ...
        if test_iteration > 0:
            ...
            loss_history["loss"].append(
                test_loss["loss"] / test_iteration
            )
            loss_history["policy"].append(
                test_loss["policy"] / test_iteration
            )
            loss_history["value"].append(
                test_loss["value"] / test_iteration
            )
